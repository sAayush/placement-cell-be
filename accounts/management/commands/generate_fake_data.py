...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('total', type=int, help='Indicates the number of fake profiles to be created')
    ...
    def handle(self, *args, **kwargs):
        ...
        for _ in range(total):
            ...
            phone_number = fake.phone_number()[:15]  # Ensure phone number length doesn't exceed 15 characters
            ...
            for _ in range(random.randint(1, 5)):
                level = random.choice([choice[0] for choice in Education.EducationLevel.choices])
                marks_or_cgpa = round(random.uniform(50, 100), 2)
                year_of_passing = fake.date_between(start_date='-10y', end_date='today')
        ...
        self.stdout.write(self.style.SUCCESS(f'{total} fake profiles created successfully'))
